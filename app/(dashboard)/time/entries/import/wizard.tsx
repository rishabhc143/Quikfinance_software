...
const TARGET_FIELDS = [
  ...
  { value: "userEmail", label: "User Email", required: true },
  { value: "hours", label: "Hours", required: true },
  { value: "billable", label: "Billable", required: false },
...
export function ImportTimesheetWizard() {
  ...
  function downloadSample(format: "csv" | "xls") {
    const blob = new Blob([SAMPLE_TIMESHEET_CSV], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `quikfinance-timesheet-sample.csv`;
    a.click();
  ...
  const requiredTargets = TARGET_FIELDS.filter((t) => t.required).map(
    (t) => t.value
  );
  ...
  async function commitImport() {
    ...
    setBusy(true);
    try {
      const result = await importTimesheetAction({
        csvText: remapped,
        dupHandling,
      });
   ...
   * what importTimesheetAction expects).
   */
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-10 bg-background border-b">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          <div />
          <h1 className="text-lg font-semibold">Timesheets - Select File</h1>
          <Link
            href="/time/entries"
            aria-label="Close"
        ...
        <div className="max-w-5xl mx-auto px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button asChild variant="outline" disabled={busy}>
              <Link href="/time/entries">Cancel</Link>
            </Button>
          </div>
        </div>
      </footer>
    ...
    <div className="space-y-8">
      ...
      >
        ...
        <p className="mt-4 text-xs text-muted-foreground">
          Maximum File Size: {MAX_FILE_SIZE_MB} MB &nbsp;•&nbsp; File Format:
          CSV or TSV or XLS
        </p>
      </div>
      ...
      <div className="grid grid-cols-[180px_1fr] gap-4 items-start">
        <div className="text-sm font-medium pt-1">
          ...
          <span
            title="How to handle entries that match an existing (user, date, project, task, hours) tuple in the database."
        ...
        <div className="space-y-3">
          <DupRadio
            ...
            description="Retains existing entries and does not import duplicate rows from the file."
          />
          <DupRadio
            checked={dupHandling === "overwrite"}
            onChange={() => setDupHandling("overwrite")}
            label="Overwrite"
            description="Updates matching entries' notes and billable flag with values from the file."
          />
        </div>
      </div>
      ...
      <div className="rounded-md border bg-amber-50/40 dark:bg-amber-950/10 p-4">
        ...
        <ul className="list-disc pl-5 space-y-1.5 text-xs text-muted-foreground">
          <li>
            Date must be in <code>yyyy-MM-dd</code> format (e.g.{" "}
            <code>2026-05-20</code>).
          </li>
          <li>
            Project Name must match an existing project (case-insensitive); Task
            Name must match a task within that project.
          </li>
          <li>
            User Email must match an existing org member&apos;s email.
          </li>
          <li>
            Hours is a decimal (e.g. <code>1.5</code> for 1h 30m). Billable
            defaults to <code>true</code>; pass <code>false</code> to mark
            non-billable.
          </li>
          <li>
            Skip Duplicates keeps existing entries; Overwrite updates notes +
            billable on matching (user/date/project/task/hours) tuples.
          </li>
        </ul>
      </div>
    </div>
