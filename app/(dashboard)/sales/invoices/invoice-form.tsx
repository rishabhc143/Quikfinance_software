...
export function InvoiceForm({
  ...
  // Persists to Invoice.taxType + Invoice.taxId via the validation
  // schema's `documentTax` discriminated union.
  const [docTaxType, setDocTaxType] = React.useState<"TDS" | "TCS" | null>(
    initial?.documentTax?.type ?? null,
  );
  const [docTaxId, setDocTaxId] = React.useState<string | null>(
    initial?.documentTax?.taxId ?? null,
  );
  ...
  async function submit(send: boolean) {
    ...
    const payload: InvoiceInput = {
      ...
      // TDS / TCS payload only when both radio + tax-id selected.
      documentTax:
        docTaxType && docTaxId
      ...
      <section className="grid gap-4 md:grid-cols-2">
        ...
        <div className="rounded-md border p-4 space-y-3 text-sm">
          ...
          <div className="space-y-2 pt-1">
            ...
            {docTaxType && taxOptions.filter((t) => (t.type ?? "standard") === docTaxType).length === 0 ? (
