...
import {
  ...
  createDeliveryMethodInlineAction,
...
export type SalesOrderFormProps = {
  ...
  // M21: optional Custom Fields wiring (entityType=SALES_ORDER)
...
export function SalesOrderForm({
  ...
  const [deliveryMethodsState, setDeliveryMethodsState] = React.useState(deliveryMethodOptions);
    ...
    <div className="space-y-6">
      <section className="rounded-md border bg-card p-6 grid gap-4 md:grid-cols-[10rem_1fr]">
        ...
        <Combobox
          ...
          onCreate={async (input: string) => {
            try {
              const created = await createDeliveryMethodInlineAction({ name: input });
              setDeliveryMethodsState((prev) => [
                ...prev,
                { value: created.id, label: created.name },
              ]);
              setDeliveryMethodId(created.id);
            } catch (err) {
              toast.error(err instanceof Error ? err.message : "Couldn't add delivery method");
            }
          }}
        />
