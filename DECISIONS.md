...
### D77. Banking v1 — CSV-only import with quadruple dedup, no auto-feeds (BNK-A).
**Choice:** Banking module v1 ships with manual CSV statement import via a 4-step wizard (Upload → Mapping → Preview → Done). No Yodlee / Plaid / Token integration. Three account types (`BANK / CREDIT_CARD / PAYPAL`) mirror Zoho's Add Bank or Credit Card form. Duplicate detection uses the quadruple `(accountId, date, amount, reference)` Zoho documents; rows with empty reference can't be deduped confidently (returned null rather than risk suppressing legitimate same-amount same-day transactions). Duplicates are still inserted with `excluded=true` so Undo Last Import can find them. UTC-based same-calendar-day comparison (not server-local) so the dedup heuristic is timezone-independent.
**Why:** the research doc at `docs/zoho-banking-research.md` (informed by 7 screenshots from the live Zoho tenant) showed Zoho's flow is 3 tiers — Partner Banks direct API + Yodlee + Manual. v1 covers tier C only. Yodlee (tier B, BNK-J) costs ~$1-2 per connected account per month and needs a vendor contract; partner-bank APIs (tier A, BNK-K) cost months of integration per bank. Starting with CSV unblocks 80% of the workflow for 5% of the integration cost, and matching/rules/reconciliation (BNK-C through BNK-F) can ship on top without re-architecting.
...
## Phase Audit — Round 1 + Round 2 sweep (PRs #318-#329)

After a senior-engineer audit (two rounds, six parallel agents in round 2), 11 PRs of structural cleanup + scale prep + bug fixes landed. Each PR's interpretive call is captured below.

### D78. `documents/actions.ts` (1850 LOC god-file) split into 6 domain-grouped subfiles, no barrel re-export (PR #319).
**Choice:** the single file got split into `actions-{upload, parse, edit-parsed, folders, delete, ar-ap}.ts` and the original `actions.ts` was **deleted entirely**. All 12 caller files updated to import from the right subfile directly.
**Why:** the first attempt kept a barrel `actions.ts` with `export * from "./actions-upload"` etc. — `pnpm build` failed because Next.js's `"use server"` strict rule forbids re-exports (only async functions can be exported from a `"use server"` file). The canonical Next.js pattern is direct imports from each subfile. Mixing concerns (upload + parse + edit + AR/AP) in one file made navigation hard and bug-isolation worse — a fix in suggested-matches risked breaking the upload pipeline because they shared helpers.
**Trade-off:** 11 caller files needed import updates (mechanical, TS-checked). No external API surface change.

### D79. Status enum constants centralised in `lib/constants/status.ts` rather than copy-pasted across 11 list pages (PR #318).
**Choice:** typed `<DOC>_STATUS_VARIANT` records mapping status strings to `StatusVariant` (success/warning/danger/info/neutral) live in one file. List pages `import { INVOICE_STATUS_VARIANT as STATUS_VARIANT }` and the rest of the page code is unchanged.
**Why:** before, each list page had its own local `const STATUS_VARIANT = {...}`. Renaming a status (e.g. `"OPEN"` → `"OPEN_BILL"`) meant a 15-file sweep with no TS safety. Centralising makes the mapping the source of truth + a typo like `"DARFT"` now fails at compile time.
**Trade-off:** the import-as-alias adds one line to each list page. Worth it for the type safety.

### D80. Composite `(organizationId, status, dueDate)` index on Bill + `(organizationId, paymentDate)` on PaymentMade (PR #318).
**Choice:** two `@@index` additions to the schema + one idempotent migration. Applied to prod via the existing `curl /api/admin/migrate` workflow (MIGRATION_KEY pulled from `vercel env pull`, never echoed).
**Why:** Reports filtering by `(organizationId, status, dueDate range)` on Bill were doing a sequential scan beyond ~10k rows per org. At current single-founder scale this is fine; at SMB scale (5k+ bills per org) the composite index keeps the query plan tight.
**Trade-off:** index storage cost is negligible. Migration is idempotent so safe to re-apply.

### D81. Razorpay webhook returns 500 on critical-path failure (PR #320) — overrides the previous "always 200" policy.
**Choice:** the outer catch in `app/api/webhooks/razorpay/route.ts` now returns `NextResponse.json({ ok: false }, { status: 500 })` instead of 200. Inside each handler, side-effects (`writeAuditLog`, `enqueueEmail`) are wrapped in inner try/catch so they don't propagate. Only critical-path DB transaction failures reach the outer catch.
**Why:** the previous policy ("Still return 200 — surface internally via logs but keep Razorpay from retrying forever") meant transient failures (DB blip, network glitch) silently lost the webhook. Razorpay never retried; the payment never got recorded; the user never got the receipt email. Razorpay's retry has exponential backoff capped at ~24h per their docs, so "retry forever" was never actually the risk. Industry-standard webhook design: 200 = processed, 4xx = bad signature, 500 = transient critical-path failure → retry.
**Trade-off:** a permanent error (schema mismatch, missing required bank account) now triggers Razorpay retries for ~24h until the backoff cap. Mitigation: log search by `[razorpay] critical-path handler error` prefix to find + fix quickly. Side-effect failures are logged with greppable `[razorpay] audit failed` / `[razorpay] email failed` prefixes so they're discoverable without triggering retries.

### D82. Bank-statement parser failures store a `_meta.parseError` sentinel inside `extractedFields` JSONB (PR #322).
**Choice:** when the heuristic + LLM fallback chain fails for a BANK_STATEMENT document, the action stores `{bank: "UNKNOWN", rows: [], _meta: { parserSource: "heuristic", parseError: "<reason>" }}`. The drawer's `BankStatementTransactionsPanel` checks `parsed._meta?.parseError` and renders an amber hint banner above the (empty) transactions table. The existing "Re-run parse with AI" button remains visible when LLM is enabled.
**Why:** before, parser failures stored `extractedFields: null` and the drawer showed an empty Transactions table — indistinguishable from "no transactions in this period" or "we didn't try to parse this." Users had no signal that Smart Capture had failed. Per CLAUDE.md fail-open philosophy the page should "always render something" — including an honest "we couldn't" banner.
**Trade-off:** zero schema migration (JSONB allows arbitrary structure). `isParsedBankStatement(...)` type guard still passes for the sentinel shape (has `bank` + `rows`).

### D83. `ContactPersonsTable` + `AddressFieldset` extracted with prop-driven divergences instead of merging them out (PRs #323/#324).
**Choice:** the shared components live in `components/shared/`. Per-form divergences (Vendor uses `<Textarea>` for address lines, Customer uses `<Input>`; different placeholder text; different copy-billing button label; lowercase vs capitalised headings) are exposed as props (`variant: "input" | "textarea"`, `countryPlaceholder?`, etc.) rather than `if (formType === "vendor")` branches inside the component.
**Why:** the two forms (Customer + Vendor) had identical structural sections that drifted slightly. Merging them via prop overrides preserves the pre-existing UX byte-for-byte and lets the next contributor add a third caller (if ever) without touching the component. Plan agent C verified Quote / SalesOrder / PurchaseOrder don't qualify — they reference contacts by `contactId` FK, not inline addresses; documented as `CURRENT CALLERS` header notes on both components.
**Trade-off:** the `form: any` prop type is intentional (react-hook-form's `UseFormReturn<T>` is invariant in `T`). Documented inline; both callers ensure `contactPersons` / `addresses` are in their schemas.

### D84. Shared zod helpers `attachmentsField(max)` + `customFieldValuesField` + reused `CustomFieldDefForRender` type (PR #325).
**Choice:** `lib/validations/shared-fields.ts` exports `attachmentSchema`, `attachmentsField(max = 10)`, and `customFieldValuesField`. Seven transaction validation files import `attachmentsField` instead of inlining the 10-line zod block. Five form files import `CustomFieldDefForRender` from `components/shared/custom-fields-section.tsx` (already existed) instead of re-declaring the 16-line type.
**Why:** the audit explicitly verified all 7 attachment fields use identical shapes (modulo `max(5)` for Quote, `max(10)` for the others) and the `customFieldDefinitions` type was redeclared 5 times. Standardised on strict bounds (Postgres VARCHAR columns enforce them anyway). Zero behaviour change.
**Trade-off:** new file in `lib/validations/`. The helper is a function (not a constant) so each caller gets a fresh zod tree — prevents accidental cross-schema mutation.

### D85. `computeDocumentTotals` lives in a new `lib/sales/document-totals.ts`, not extended into `lib/sales/totals.ts` (PR #325 + #326).
**Choice:** the shared "fetch taxes + map lines + call `computeDocument`" wrapper used by 10 server actions is exported from `lib/sales/document-totals.ts`, a new file that imports both `lib/db` and `lib/sales/totals`. The original `totals.ts` stays pure (no Prisma dependency).
**Why:** the Plan agent's first design proposed adding `computeDocumentTotals` to `totals.ts` itself. Reviewing: `totals.ts` is intentionally pure — runs in tests, scripts, the LLM parser fallback, anywhere. Importing `db` into it would break those callers. Separating the Prisma-dependent wrapper from the pure math is the right cut. The wrapper accepts a superset input shape so callers that don't have `documentDiscount` / `documentTax` / `adjustmentValue` (credit-notes, debit-notes, delivery-challans) pass an input without those keys.
**Trade-off:** one extra file. The recurring-bills caller keeps the `computeDocument` import alongside the new helper because it uses `ReturnType<typeof computeDocument>` as a type in `buildTemplateJson`.

### D86. Cron status sweeps use `$executeRaw UPDATE` instead of Prisma's `updateMany` (PR #327).
**Choice:** `app/api/cron/{invoice,bill}-statuses/route.ts` now executes one raw SQL `UPDATE` per cron run. The predicate `"total" > "amountPaid"` is column-vs-column which Prisma's `updateMany` can't express in its WHERE clause.
**Why:** before, the cron did `findMany → JS loop → individual update()` per row — 10k rows = 10k sequential Neon round-trips, ~25s of the 60s Vercel function budget. Hit the timeout around 25k rows. The single SQL `UPDATE` runs in <100ms even at 100k rows because Postgres can do the predicate evaluation + the column update in one indexed scan. Idempotent: the where-clause excludes already-OVERDUE rows.
**Trade-off:** the new response drops the previous `scanned` field (we don't load rows into memory anymore — just return `updated` rowcount from `$executeRaw`'s return value). Cron consumers are HTTP-only and don't parse the body.

### D87. `mapPool` concurrency = 8 for the recurring crons (PR #327).
**Choice:** `lib/concurrency.ts` exports `mapPool(items, concurrency, fn)` — a tiny bounded-concurrency helper. The 3 recurring crons (`invoices`/`bills`/`expenses`) use `mapPool(due, 8, generateOccurrence)` in place of the sequential for-await loop.
**Why:** Neon's WebSocket adapter pool max is approximately 10 connections; 8 leaves headroom for sibling warm functions in the same region. Crons in `vercel.json` are staggered 15 min apart so cross-cron contention is rare. Before, 200 profiles processed sequentially took ~20s — no margin at 500+ profiles. With concurrency=8, the same 200 profiles finish in ~2.5s. Also added inner try/catch to `recurring-invoices/route.ts` for parity with bills + expenses (one bad profile no longer aborts the whole run).
**Trade-off:** one extra file in `lib/`. 7 unit tests in `tests/unit/concurrency.test.ts` cover empty input, zero concurrency, order preservation, concurrency-limit, error propagation, worker clamping, index passing.

### D88. Email retry exponential backoff via `scheduledFor` push, not a separate retry-schedule table (PR #327).
**Choice:** in `lib/sales/email-sender.ts`'s `processEmailJob` catch block, push `scheduledFor` forward by `RETRY_BACKOFF_MS[nextAttempts]` indexed by the post-increment attempt count: `{1: 1m, 2: 5m, 3: 30m, 4: 6h}`. After 5 attempts the job is marked `FAILED` with no further retry. `drainPendingEmails` already filters by `scheduledFor: { lte: now }` so future-dated jobs naturally skip until the backoff elapses.
**Why:** before, failed jobs kept their original `scheduledFor` so every cron tick burned through the same 50-row batch — when the email service was down for a day, the queue thrashed and logs were spammed with the same 50 errors over and over. Reusing the existing `scheduledFor` column avoids a schema migration. The backoff curve `[1m, 5m, 30m, 6h]` ~= 30 hours total, matches Stripe / Resend / SendGrid common retry curves.
**Trade-off:** the `email-job-retry` cron is daily on Hobby plan — for the backoff to actually fire its short steps the schedule needs to be sub-daily (`*/15 * * * *`), which requires Vercel Pro ($20/mo). Documented as optional follow-up. On daily-only, the backoff still prevents the cron-tick thrash but the actual retries land at next-day intervals.

### D89. `parseListSearchParams` returns `sort` as `string`, not a generic literal type (PR #328).
